    ...
    public class OpenTKApp : GameWindow
    {
        ...
        protected override void OnLoad()
        {
            ...
            {   // setting up a Modern OpenGL pipeline takes a lot of code
                ...
                this.KeyDown += (KeyboardKeyEventArgs e) =>
                {
                    if (e.Key == Keys.M)
                    {
                    ...
                    if (e.Key == Keys.N)
                    {
                        app.debugData = !app.debugData;
                    }
                    if (e.Key == Keys.O)
                    {
                        app.camera.orientationLock = !app.camera.orientationLock;
                    }
                };
        ...
        protected override void OnUpdateFrame(FrameEventArgs e)
        {
            ...
            Vector3 leftDirection = Vector3.Cross(app.camera.lookAtDirection, app.camera.upDirection);

            if ((MouseState.Delta[0] != 0 || MouseState.Delta[1] != 0) && app.camera.orientationLock == false)
            {
                float sensitivity = 0.0005f;
                // de yaw en pitch moeten nog geclamped worden
                app.camera.yaw += MouseState.Delta[0] * sensitivity;
                app.camera.pitch -= MouseState.Delta[1] * sensitivity;

                Quaternion yawQuat = new Quaternion(app.camera.standardUpDirection, app.camera.yaw);
                app.camera.lookAtDirection = (yawQuat * new Quaternion(app.camera.standardLookAtDirection, 0) * yawQuat.Inverted()).Xyz;

                leftDirection = Vector3.Cross(app.camera.lookAtDirection, app.camera.standardUpDirection);
                Quaternion pitchQuat = new Quaternion(leftDirection, app.camera.pitch);

                app.camera.lookAtDirection = (pitchQuat * new Quaternion(app.camera.lookAtDirection, 0) * pitchQuat.Inverted()).Xyz;
                app.camera.upDirection = -1 * (pitchQuat * new Quaternion(app.camera.standardUpDirection, 0) * pitchQuat.Inverted()).Xyz;


            }

            float movementDelta = 15;
            if (KeyboardState[Keys.W])
            {
                app.camera.position += movementDelta * app.camera.lookAtDirection;
            }
            if (KeyboardState[Keys.A])
            {
                app.camera.position -= movementDelta * leftDirection;
            }
            ...
            {
                app.camera.position += movementDelta * app.camera.upDirection;
            }
            if (KeyboardState[Keys.LeftShift])
            {
                app.camera.position -= movementDelta * app.camera.upDirection;
            }

            float focalLengthDelta = 3;
            if (KeyboardState[Keys.R])
            {
                app.camera.focalLength += focalLengthDelta;
            }
            if (KeyboardState[Keys.F])
            {
                if (app.camera.focalLength - focalLengthDelta > 0)
                {
                    app.camera.focalLength -= focalLengthDelta;
                }
                else
                {
                    app.camera.focalLength = 0;
                }
            }
